...
OptionTradeBias = Literal["bullish", "bearish", "neutral"]
OptionTradeIdeaStatus = Literal["candidate_ready", "watch", "not_applicable"]
OptionTradeIdeaMode = Literal["planning", "live"]
OptionDataQuality = Literal["provider", "estimated", "fallback", "unavailable"]
OptionsProviderName = Literal["alpaca", "disconnected"]
OptionsProviderState = Literal["connected", "not_connected", "integration_pending"]
...
class ImpliedVolatilitySnapshot(BaseModel):
    value: float = Field(ge=0)
    rank_percentile: int | None = Field(default=None, ge=0, le=100)
    source: str
...
class OptionContractSnapshot(BaseModel):
    ...
    implied_volatility: ImpliedVolatilitySnapshot | None = None
...
class OptionSelectionPreferences(BaseModel):
    min_days_to_expiration: int = Field(ge=0)
    target_days_to_expiration: int = Field(ge=0)
    max_days_to_expiration: int = Field(ge=0)
    min_open_interest: int = Field(ge=0)
    max_spread_bps: int = Field(gt=0)
    target_delta: float = Field(gt=0, le=1)
    premium_budget: float = Field(gt=0)
    defined_risk_preference: bool = True

    @model_validator(mode="after")
    def validate_dte_window(self) -> "OptionSelectionPreferences":
        if self.min_days_to_expiration > self.target_days_to_expiration:
            raise ValueError("target_days_to_expiration must be >= min_days_to_expiration")
        if self.target_days_to_expiration > self.max_days_to_expiration:
            raise ValueError("max_days_to_expiration must be >= target_days_to_expiration")
        return self


class OptionsProviderStatus(BaseModel):
    provider: OptionsProviderName
...
class OptionSelectionScoreComponent(BaseModel):
    label: Literal["direction", "dte", "liquidity", "spread", "delta", "premium", "defined_risk"]
...
class ContractSelectionRationale(BaseModel):
    summary: str
    reasons: list[str]
    warnings: list[str]
    score_components: list[OptionSelectionScoreComponent]


class OptionTradeIdea(BaseModel):
    generated_at: datetime
    status: OptionTradeIdeaStatus
    mode: OptionTradeIdeaMode
    underlying_symbol: str
    underlying_price: float = Field(gt=0)
    underlying_action: SignalAction
    bias: OptionTradeBias
    candidate_count: int = Field(ge=0)
    selection_score: float = Field(ge=0, le=100)
    provider_status: OptionsProviderStatus
    selection_preferences: OptionSelectionPreferences
    selected_contract: OptionContract | None = None
    selected_snapshot: OptionContractSnapshot | None = None
...
class PaperExecutionAssumptions(BaseModel):
    ...
    supported_instruments: list[Literal["equity"]]
    position_unit: Literal["shares"]
    options_execution_supported: bool
...
class TradingSignalRecord(BaseModel):
    ...
    options_trade_idea: OptionTradeIdea | None = None
