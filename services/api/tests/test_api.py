...
from app.providers.options import AlpacaOptionsProvider, DisconnectedOptionsProvider, OptionsProviderNotConnectedError
...
from app.services.options import OptionsDataGateway, rank_contract_candidates
...
def clean_database() -> None:
    ...
    options_service.reset_options_data_gateway()
...
def build_option_candidate(
    *,
    ...
    mark = round(ask - (spread_width / 2), 2)
    bid = round(max(0.01, ask - spread_width), 2)
    spread_bps = int(round((spread_width / mark) * 10_000)) if mark else 0
    return OptionContractSnapshot(
        contract=OptionContract(
            contract_symbol=(
                f"{symbol}{expiry_date.strftime('%y%m%d')}"
                f"{'C' if side == 'call' else 'P'}"
                f"{int(round(strike_price * 1000)):08d}"
            ),
            underlying_symbol=symbol,
            side=side,  # type: ignore[arg-type]
            strike=OptionStrike(price=strike_price, distance_percent=0.0),
            expiration=OptionExpiration(date=expiry_date, days_to_expiration=dte)
        ),
        as_of=datetime.now(timezone.utc),
        source="unit-test",
        quality="estimated",
        ...
        greeks=OptionGreeks(delta=delta, gamma=0.02, theta=-0.08, vega=0.11, rho=0.02)
    )
...
def test_option_model_validation_guards_invalid_dte_and_quotes() -> None:
...
def test_options_provider_interface_reports_not_connected_state() -> None:
    disconnected_gateway = OptionsDataGateway(DisconnectedOptionsProvider())
    disconnected_status = disconnected_gateway.get_provider_status()
    assert disconnected_status.state == "not_connected"
    assert disconnected_status.live_requests_enabled is False

    with pytest.raises(OptionsProviderNotConnectedError):
        disconnected_gateway.get_expirations("NVDA")

    alpaca_provider = AlpacaOptionsProvider(
        api_key=None,
        secret_key=None,
        base_url="https://paper-api.alpaca.markets"
    )
    alpaca_status = alpaca_provider.get_status()
    assert alpaca_status.provider == "alpaca"
    assert alpaca_status.state == "not_connected"
    assert alpaca_status.live_requests_enabled is False


def test_option_candidate_ranking_prefers_balanced_liquid_contract() -> None:
    ...
    ranked = rank_contract_candidates(candidates, preferences, "bullish")

    assert ranked[0].snapshot.contract.strike.price == 985
    assert ranked[0].score > ranked[-1].score
    assert ranked[0].snapshot.liquidity.open_interest.value >= preferences.min_open_interest


def test_signal_response_includes_planning_mode_option_trade_idea(
    client: TestClient,
    ...
    option_idea = nvda_signal["options_trade_idea"]

    assert option_idea is not None
    assert option_idea["mode"] == "planning"
    assert option_idea["provider_status"]["state"] in {"not_connected", "integration_pending"}
    assert option_idea["selected_contract"] is not None
    assert option_idea["selected_snapshot"]["liquidity"]["open_interest"]["value"] > 0
