...
export interface ImpliedVolatilitySnapshot {
  value: number;
  rank_percentile: number | null;
  source: string;
  quality: "provider" | "estimated" | "fallback" | "unavailable";
}
...
export interface OptionLiquiditySnapshot {
  bid: number | null;
  ask: number | null;
  mark: number | null;
  last: number | null;
...
export interface OptionContractSnapshot {
  ...
  implied_volatility: ImpliedVolatilitySnapshot | null;
...
export interface OptionSelectionPreferences {
  min_days_to_expiration: number;
  target_days_to_expiration: number;
  max_days_to_expiration: number;
  min_open_interest: number;
  max_spread_bps: number;
  target_delta: number;
  premium_budget: number;
  defined_risk_preference: boolean;
}

export interface OptionsProviderStatus {
  provider: "alpaca" | "disconnected";
  state: "connected" | "not_connected" | "integration_pending";
  configured: boolean;
  live_requests_enabled: boolean;
  message: string;
}

export interface OptionSelectionScoreComponent {
  label: "direction" | "dte" | "liquidity" | "spread" | "delta" | "premium" | "defined_risk";
  weight: number;
  score: number;
  contribution: number;
  detail: string;
...
export interface ContractSelectionRationale {
  summary: string;
  reasons: string[];
  warnings: string[];
  score_components: OptionSelectionScoreComponent[];
}

export interface OptionTradeIdea {
  generated_at: string;
  status: "candidate_ready" | "watch" | "not_applicable";
  mode: "planning" | "live";
  underlying_symbol: string;
  underlying_price: number;
  underlying_action: "BUY" | "SELL" | "HOLD" | "EXIT" | "REDUCE" | "NO_TRADE";
  bias: "bullish" | "bearish" | "neutral";
  candidate_count: number;
  selection_score: number;
  provider_status: OptionsProviderStatus;
  selection_preferences: OptionSelectionPreferences;
  selected_contract: OptionContract | null;
  selected_snapshot: OptionContractSnapshot | null;
...
export interface PaperExecutionAssumptions {
  ...
  options_execution_supported: boolean;
}
...
export interface TradingSignalRecord {
  ...
  options_trade_idea: OptionTradeIdea | null;
}
