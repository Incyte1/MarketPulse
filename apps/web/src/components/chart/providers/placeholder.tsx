...
export function PlaceholderChart({ model }: ChartProviderProps) {
  const points = toPolyline(buildSeries(model.symbol));
  const isEmbedded = model.renderMode === "embedded";
    ...
    <div className="chartProviderPlaceholder">
      ...
      {!isEmbedded ? (
        <div className="chartCapabilityList">
          ...
          <span className="tag">Fallback chart</span>
          <span className="tag">Local/dev safe</span>
        </div>
      ) : null}
