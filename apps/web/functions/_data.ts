...
function buildFallbackOptionCandidate({
  action,
  bias,
  dte,
  now,
  ...
  now: string;
  side: "call" | "put";
  status: "candidate_ready" | "watch";
  strike: number;
  symbol: string;
  underlyingPrice: number;
}) {
  const expirationDate = new Date(Date.now() + dte * 24 * 60 * 60 * 1000)
    .toISOString()
    .slice(0, 10);
  const premium = side === "call" ? 4.85 : 3.9;
  const contractSymbol = `${symbol}${expirationDate.replaceAll("-", "").slice(2)}${side === "call" ? "C" : "P"}${String(Math.round(strike * 1000)).padStart(8, "0")}`;
  ...
  return {
    ...
    provider_status: {
      ...
      message:
        "Alpaca options is not connected yet. This contract is a planning-only scaffold until provider wiring is enabled."
    },
    ...
    selected_contract: {
      ...
      strike: {
        price: strike,
        distance_percent: Number((((strike / underlyingPrice) - 1) * 100).toFixed(2))
      },
      expiration: {
        date: expirationDate,
    ...
    selected_snapshot: {
      ...
      greeks: {
        delta: side === "call" ? 0.36 : -0.34,
        gamma: 0.022,
        theta: -0.095,
        vega: 0.14,
        rho: side === "call" ? 0.02 : -0.02
      }
    },
    rationale: {
      summary:
        status === "watch"
          ? `Prepared ${contractSymbol} as a watch candidate while the underlying setup still needs confirmation.`
          : `Selected ${contractSymbol} as the cleanest ${bias} planning contract under the current DTE, delta, liquidity, spread, and premium constraints.`,
      reasons: [
        "Contract side matches the underlying directional bias.",
        "DTE stays inside the short-duration window for a one-trade-per-day options workflow.",
        "Open interest and quoted spread are strong enough for planning purposes.",
        "The contract keeps premium inside the current budget guardrail."
      ],
      warnings: [
        "Provider not connected yet. Pricing, greeks, and liquidity are planning-only estimates.",
        "This scaffold intentionally stays in long premium only; multi-leg defined-risk structures are not modeled yet."
      ],
      score_components: [
        {
          label: "direction",
          ...
          score: 100,
          contribution: 18,
          detail: "Contract side matches the underlying directional bias."
        },
        {
          label: "dte",
          weight: 0.17,
          score: 88,
          contribution: 14.96,
          detail: "DTE is close to the one-week target."
        },
        {
          label: "liquidity",
          weight: 0.18,
          score: 84,
          contribution: 15.12,
          detail: "Open interest and volume are workable for a retail-sized single-contract entry."
        },
        {
          label: "spread",
          weight: 0.14,
          score: 86,
          contribution: 12.04,
          detail: "Quoted spread stays inside the current cap."
        },
        {
          label: "delta",
          weight: 0.18,
          score: 92,
          contribution: 16.56,
          detail: "Delta is close to the directional target."
        },
        {
          label: "premium",
          weight: 0.1,
          score: 78,
          contribution: 7.8,
          detail: "Estimated premium remains inside the working budget."
        },
        {
          label: "defined_risk",
          weight: 0.05,
          score: 100,
          contribution: 5,
          detail: "Only long premium is considered in fallback planning mode."
        }
      ]
    }
  };
...
function buildFallbackOptionInactive({
  action,
  now,
  symbol,
  underlyingPrice
}: {
  action: "HOLD" | "EXIT" | "REDUCE" | "NO_TRADE";
  now: string;
  symbol: string;
  underlyingPrice: number;
}) {
  return {
    ...
    provider_status: {
      ...
      live_requests_enabled: false,
      message:
        "Alpaca options is not connected yet. Unveni is showing planning surfaces only."
    },
    selection_preferences: {
      min_days_to_expiration: 3,
      target_days_to_expiration: 10,
      max_days_to_expiration: 28,
    ...
    rationale: {
      ...
      reasons: [
        "The current intraday signal is standing aside rather than preparing a fresh options entry."
      ],
      warnings: [
        "Provider not connected yet. Live chain selection is still pending Alpaca integration."
      ],
      score_components: []
    }
  };
    ...
    items: [
      {
        ...
        options_trade_idea: buildFallbackOptionCandidate({
          action: "NO_TRADE",
          bias: "bullish",
          dte: 7,
          now,
      ...
      },
      {
        ...
        options_trade_idea: buildFallbackOptionInactive({
          action: "NO_TRADE",
          now,
          symbol: "AAPL",
          underlyingPrice: 191.3
        })
      }
    ]
  };
